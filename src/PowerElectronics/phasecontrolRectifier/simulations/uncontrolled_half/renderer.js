...
let chart1data = [], chart2data = [], chart3data = [], chart4data = [];
let sliderVal;
let numPoints;
...
function generateChartData(sliderVal) {
  ...
  chart2data = Array.from({ length: numPoints }, (_, i) => {
    ...
    return isOddCycle ? Math.sin(xValue) : 0;
  });
  ...
  chart3data = Array.from({ length: numPoints }, (_, i) => {
    ...
    if (isOddCycle && positionInCycle <= oddCycleRange) {
      return 0;
    } else if (isOddCycle) {
      return Math.sin(xValue);
    } else {
...
function updateCharts() {
  const colors = ['rgba(153, 102, 255, 1)', 'rgba(255, 99, 132, 1)', 'rgba(75, 192, 192, 1)', 'rgba(255, 153, 0, 1)'];
  const titles = ["Vs", "Vo", "Io", "Vd"];
  const chartData = [chart1data, chart2data, chart3data, chart4data];
  const yRanges = [[-1, 1], [-0.01, 1.00001], [-0.01, 1.00001], [-1, 1]];

  [chart1, chart2, chart3, chart4].forEach((chart, i) => {
