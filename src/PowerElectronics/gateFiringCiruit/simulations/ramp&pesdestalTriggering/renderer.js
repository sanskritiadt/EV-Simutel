...
function generateChartData(sliderVal) {
  ...
  chart3data = Array.from({ length: numPoints }, (_, i) => {
    ...
    if (positionInCycle >= 0.3 * Math.PI && positionInCycle <= 0.7 * Math.PI) {
      return Math.sin(0.3 * Math.PI);
    } else {
      return Math.abs(Math.sin(xValue));
    }
  });
  ...
  chart5data = Array.from({ length: numPoints }, (_, i) => {
    ...
    if (1 - positionInCycle / Math.PI < Math.max(0, sliderVal * 0.01)) {
      return Math.sin(xValue);
    } else {
      return 0
    }
